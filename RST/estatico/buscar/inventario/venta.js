...
function board(data,pag){
	...
	html+="<th>cantidad</th><th>tarjeta</th><th>agregar</th></tr>";
	for(i=0;i<data.length;i++){
		...
		if(ver_distribuidor==true){html+="<td>"+data[i].distribuidor+"</td>"}
		if(ver_mayorista==true){html+="<td>"+data[i].mayorista+"</td>"}
		if(ver_efectivo==true){html+="<td>"+data[i].efectivo+"</td>"}
		html+="<td><input type='text' id='stock_"+data[i].id+"' placeholder='stock: "+data[i].cantidad+"'></td>"
		html+="<td><input type='text' id='precio_"+data[i].id+"' value='"+data[i].tarjeta+"'></td>"
		html+="<td><button class='btn-success' onclick='detalle("+data[i].id+")'>+</button></td></tr>";
	}
	var prev="", next="";
	if (pag!=0){
		p = pag-1;
